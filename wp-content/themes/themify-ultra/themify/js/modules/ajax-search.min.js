...
b.style.display="none");if(48<=c.keyCode&&57>=c.keyCode||65<=c.keyCode&&90>=c.keyCode||8===c.keyCode||229===c.keyCode){let c=this.value.trim();if(c){const e=l(b.querySelector(".search-option-tab li.active"));m[c]?(b.style.display="none",b.innerHTML=m[c],p(e)):setTimeout(function(){c=g.value.trim();h&&k.abort();if(c){var d=new Headers({Accept:"application/json","X-Requested-With":"XMLHttpRequest"}),f=new FormData;f.append("action","themify_search_autocomplete");f.append("term",c);f.append("post_type",
a.post_type?a.post_type:"");a.container.classList.add("themify-loading");h=!0;k=new AbortController;fetch(themify_vars.ajax_url,{signal:k.signal,method:"POST",headers:d,body:f}).then(function(a){return a.text()}).then(d=>{c?d&&(b.style.display="none",b.innerHTML=d,p(e),m[c]=d):b.innerHTML="";a.container.classList.remove("themify-loading");h=!1}).catch(a=>{"AbortError"!==a.name&&console.error("Uh oh, an error!",a)})}else b.innerHTML=""},100)}else b.innerHTML=""}},{passive:!0})}};c.on("themify_overlay_search_init",
function(a){r(a)})})(Themify);
