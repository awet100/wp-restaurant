...
(function ($, Themify, document, window, themifyScript) {
    ...
    const updateFullPage = function(options){
        ...
        createFullScrolling = function(options){
            ...
            if(rows && rows[0]){
				if($body[0].classList.contains( 'fullpage-footer' ) ){
					const footer = document.getElementById( 'footerwrap' );
					if (footer) {
						footer.classList.add('module_row');
                ...
                for(let i = 0,len = rows.length; i < len; ++ i){
                    if ( null===rows[i].previousSibling ) {
						if ( rows[i].getAttribute( 'id' ) === 'footerwrap' ) {
							sectionAnchors.push( '' );
						} else if ( rows[i].getAttribute( 'data-row-title' ) ) {
							sectionAnchors.push( rows[i].getAttribute( 'data-row-title' ) );
						} else {
            ...
            $wrapper.fullpage({
                ...
                afterRender:function(){
					if(initOnce===false){
						...
						const changeHash = function(hash){
							if(hash && hash !== '#' && (prevHash !== hash || window.location.hash!==hash)){
								prevHash = hash;
						...
						if(rows && rows[0]){
							for(let i = rows.length-1; i>-1;--i){
								let bg=rows[i].getElementsByClassName('fp-bg')[0];
								if(!bg){
									bg=document.createElement('div');
									bg.className='fp-bg';
									rows[i].prepend(bg);
								}
								const temp = rows[i].dataset['lazy'];
								if(temp){
								    delete rows[i].dataset['lazy'];
                                }
								const bgImg = getComputedStyle(rows[i]).backgroundImage.toLowerCase();
								if(bgImg.indexOf('.png')!==-1 || bgImg.indexOf('.svg')!==-1){
                                    bg.style.backgroundAttachment='fixed';
                                }
                                if(temp){
                                    rows[i].dataset['lazy']=temp;
                                }
							}
						}
                ...
                onSlideLeave:function(anchorLink,sectionIndex,prevSlideIndex,direction,nextIndex,index){
                    ...
                    if($body[0].classList.contains('themify_mp_opened')){
                        return false;
                    }
                    const sections = this[0].parentNode.children;
                    for(let i = sections.length - 1; i > - 1; -- i){
